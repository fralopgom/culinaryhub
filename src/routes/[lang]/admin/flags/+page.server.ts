...
export const load: PageServerLoad = async ({ locals }) => {
	...
	const flagged = await db`
		SELECT
			...
			u.address AS author_address, u.username AS author_username,
		...
		GROUP BY r.id, r.slug, r.title, r.status, u.address, u.username
...
export const actions: Actions = {
	confirm_flags: async ({ request, locals }) => {
		...
		await logAction(locals.user!.id, 'hide_recipe', recipe_id, reason);
		return { success: true };
	},
	...
	dismiss_flags: async ({ request, locals }) => {
		...
		await logAction(locals.user!.id, 'dismiss_flags', recipe_id, reason);
		return { success: true };
	},
	...
	ban_wallet: async ({ request, locals }) => {
		...
		const [u] = await db`SELECT id FROM users WHERE address = ${address}`;
		...
		await logAction(locals.user!.id, 'ban_wallet', u.id, reason);
		return { success: true };
	}
};
